...
const handleRequest = async (method, url, data) => {
  ...
  const getDB = (key) => JSON.parse(localStorage.getItem(DB_PREFIX + key) || '[]');
  const setDB = (key, arr) => localStorage.setItem(DB_PREFIX + key, JSON.stringify(arr));
  ...
  if (path === '/settings/system') {
    if (method === 'PUT') {
      localStorage.setItem('zz_system_pricing', JSON.stringify(data));
      return { data: { success: true, data } };
    }
    const raw = localStorage.getItem('zz_system_pricing');
  ...
  if (path === '/logistics/pricing') {
    const raw = localStorage.getItem('zz_shipping_mode_pricing_v1');
...
// ==========================================
// AXIOS COMPATIBLE INTERCEPTOR IMPLEMENTATION
// ==========================================
const mockAxios = {
  create: () => mockAxios,
  interceptors: {
  ...
  get: (url, config) => handleRequest('GET', url, null),
...
// ==========================================
// GLOBAL fetch() OVERRIDE FOR OFFLINE MOCKING
// ==========================================
if (typeof window !== "undefined") {
  ...
  window.fetch = async (input, init) => {
    ...
    // 1. Intercept system pricing settings API calls
    if (urlStr.includes('/settings/system')) {
      if (init && init.method === 'PUT') {
        ...
        const raw = localStorage.getItem('zz_system_pricing');
        const data = raw ? JSON.parse(raw) : { chauffeur_base_price: '50.00', delivery_base_price: '25.00', pickup_charges: '10.00', per_km_charges: '2.50' };
        return new Response(JSON.stringify({ success: true, data }), { 
          status: 200, 
          headers: { 'Content-Type': 'application/json' } 
        });
    ...
    // 2. Intercept landing public admins
    if (urlStr.includes('/api/staff/public/admins')) {
      const data = [{ id: 'admin1', name: 'ZaneZion HQ Admin', email: 'admin@zanezion.com' }];
      return new Response(JSON.stringify({ success: true, data }), { 
        status: 200, 
        headers: { 'Content-Type': 'application/json' } 
    ...
    // 3. Intercept OpenStreetMap Geocode and OSRM Routes
    if (urlStr.includes('nominatim.openstreetmap.org')) {
      const mockGeocode = [{ lat: '25.06', lon: '-77.34', display_name: 'Nassau, Bahamas' }];
      return new Response(JSON.stringify(mockGeocode), { 
        status: 200, 
        headers: { 'Content-Type': 'application/json' } 
      });
    }
    ...
    if (urlStr.includes('router.project-osrm.org')) {
      const mockRoute = {
        routes: [{ distance: 5400, duration: 720 }]
      };
      return new Response(JSON.stringify(mockRoute), { 
        status: 200, 
        headers: { 'Content-Type': 'application/json' } 
      });
    }

    // Pass through to original fetch for local Vite assets
